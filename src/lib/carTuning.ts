...
export function clampTuning(tuning: CarHandlingConfig): CarHandlingConfig {
	return {
		topSpeed: Math.max(30, Math.min(500, tuning.topSpeed)),
		acceleration: Math.max(30, Math.min(520, tuning.acceleration)),
		steering: Math.max(1.5, Math.min(25, tuning.steering)),
